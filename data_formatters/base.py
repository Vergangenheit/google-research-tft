...
class GenericDataFormatter(abc.ABC):
  ...
  """
    ...
    @abc.abstractmethod
    def format_predictions(self, df: DataFrame):
    ...
    @property
    def num_classes_per_cat_input(self) -> int:
    ...
    def _get_tft_input_indices(self) -> Dict:
        """Returns the relevant indexes and input sizes required by TFT."""

        # Functions
        def _extract_tuples_from_data_type(data_type, defn) -> List[Tuple]:
            return [
                tup for tup in defn if tup[1] == data_type and
                                       tup[2] not in {InputTypes.ID, InputTypes.TIME}
            ]

        def _get_locations(input_types, defn) -> List:
