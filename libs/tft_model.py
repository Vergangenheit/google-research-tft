...
def apply_mlp(inputs: Union[Input, Tensor],
              hidden_size: int,
              output_size: int,
...
def apply_gating_layer(x: Union[Input, Tensor],
...
def gated_residual_network(x: Union[Input, Tensor],
                           hidden_layer_size: int,
    ...
    if output_size is None:
        ...
        if use_time_distributed:
            linear = tf.keras.layers.TimeDistributed(linear)
        skip: Tensor = linear(x)

    # Apply feedforward network
    hidden: Tensor = linear_layer(
        hidden_layer_size,
        activation=None,
...
def get_decoder_mask(self_attn_inputs: Union[Tensor, Input]) -> Tensor:
    ...
    mask: Tensor = K.cumsum(tf.eye(len_s, batch_shape=bs), 1)
    return mask
...
class ScaledDotProductAttention:
    ...
    def __call__(self, q, k, v, mask) -> (Tensor, Tensor):
        ...
        attn = Lambda(lambda x: K.batch_dot(x[0], x[1], axes=[2, 2]) / temper)(
            [q, k])  # shape=(batch, q, k)
        if mask is not None:
            mmask: Tensor = Lambda(lambda x: (-1e+9) * (1. - K.cast(x, 'float32')))(
                mask)  # setting to infinity
            attn = Add()([attn, mmask])
        attn = self.activation(attn)
        attn: Tensor = self.dropout(attn)
        output: Tensor = Lambda(lambda x: K.batch_dot(x[0], x[1]))([attn, v])
        return output, attn
...
class InterpretableMultiHeadAttention:
    ...
    def __call__(self, q, k, v, mask=None) -> (Tensor, Tensor):
        ...
        attn: Tensor = K.stack(attns)

        outputs = K.mean(head, axis=0) if n_head > 1 else head
        outputs = self.w_o(outputs)
        outputs: Tensor = Dropout(self.dropout)(outputs)  # output dropout
...
class TemporalFusionTransformer(object):
    ...
    def _get_single_col_by_type(self, input_type: InputTypes):
    ...
    def fit(self, train_df=None, valid_df=None):
        ...
        if os.path.exists(os.path.join(self.data_folder, 'data.npy')) is False:
            ...
            # save arrays
            save(os.path.join(self.data_folder, 'data.npy'), data)
            save(os.path.join(self.data_folder, 'labels.npy'), labels)
            save(os.path.join(self.data_folder, 'active_flags.npy'), active_flags)
        ...
        training_dataset = training_dataset.repeat(self.num_epochs)
        self.valid_dataset = self.valid_dataset.repeat(self.num_epochs)
    ...
    @classmethod
    def get_hyperparm_choices(cls) -> Dict:
