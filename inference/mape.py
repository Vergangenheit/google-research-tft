...
def process_mape_by_hz(targets_df: DataFrame, preds_df: DataFrame, hours_mape: int, hz: str, time_col: str, up_col: str) -> DataFrame:
    """
    ...
    """
    df_mape: DataFrame = pd.DataFrame(
        data={time_col: preds_df[time_col], 'identifier': preds_df['identifier'], 'true': targets_df[hz],
              'preds': preds_df[hz]})
    df_mape['abs(Pred-true)']: Series = np.abs(df_mape['preds'] - df_mape['true'])
    # loop over up
    count_up = 0
    for up in targets_df[up_col].unique():
        if count_up == 0:
            ...
            p: List = []
            for i in range(0, df_mape_up.shape[0] - hours_mape):
                f: Union[str, Timestamp] = df_mape_up[time_col].iloc[i]
                p.append(f)
            #         assert len(p) == len(d)
...
def rolling_mape(targets_df: DataFrame, preds_df: DataFrame, hours_mape: int, time_col: str, up_col: str) -> DataFrame:
    """
    ...
    for mt in targets_df.columns[2:]:
        if count == 0:
            df_mape_final: DataFrame = process_mape_by_hz(targets_df, preds_df, hours_mape, mt, time_col, up_col)
            count += 1
        else:
            df_mape_interim: DataFrame = process_mape_by_hz(targets_df, preds_df, hours_mape, mt, time_col, up_col)
            df_mape_final[f'mape_{mt}'] = df_mape_interim[f'mape_{mt}']
            count += 1
